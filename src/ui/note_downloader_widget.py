...
import requests             # For session creation
import yaml                 # For config loading
import urllib.parse         # For joining URLs
from pathlib import Path    # For path handling

from PyQt6.QtCore import Qt, QDir, QUrl, QThread, pyqtSignal, QTimer # Add QThread, pyqtSignal, QTimer
from PyQt6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QPushButton, # Add QHBoxLayout
                             QPlainTextEdit, QMessageBox, QLabel, QSplitter) # Add QLabel and QSplitter
# Import requests.utils for cookiejar manipulation
import requests.utils
# QNetworkCookie might not be directly used now, but keep for reference if needed later
from PyQt6.QtNetwork import QNetworkCookie
try:
    # 只有你想把 Moodle 登录页面嵌进来时才需要
...
class NoteDownloaderWidget(QWidget):
    """嵌入式 Note-Downloader 界面（WebEngine 登录 + QThread 下载）"""
    # Signals for worker thread
    log_message = pyqtSignal(str)

    def __init__(self, project_root: str, parent=None):
        super().__init__(parent)
        self.project_root = project_root # Path to note_downloader directory
        self.session: requests.Session | None = None # requests session with cookies
        self.worker: DownloadWorker | None = None # Worker thread instance
        self.cfg: dict | None = None # Loaded config.yaml content
        self.base_url: str | None = None # Moodle base URL from config

        layout = QVBoxLayout(self)
        layout.setSpacing(6)
        layout.setContentsMargins(8, 8, 8, 8)

        # 3️⃣ Log Output Area (Create log widget first for signal connection)
        self.log = QPlainTextEdit(self)
        self.log.setReadOnly(True)
        self.log.setMaximumBlockCount(2000)       # 限制行数，防止无限增长
        self.log_message.connect(self._append) # Connect log signal here

        # === Use QSplitter for Vertical Layout ===
        splitter = QSplitter(Qt.Orientation.Vertical, self)

        # 4️⃣ Embedded Browser (Optional)
        if WEB_OK:
            self.web = QWebEngineView(self)
            self.web.setVisible(True) # Start visible
            splitter.addWidget(self.web)          # Add browser first (top)
            self.web.loadFinished.connect(self._on_page_load_finished) # Connect load finished signal
        else:
            no_web_label = QLabel("错误：PyQtWebEngine 未安装或无法加载。\n无法使用内嵌浏览器进行登录。", self)
            no_web_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
            splitter.addWidget(no_web_label) # Add label to splitter if web engine fails

        # Add log widget to splitter (bottom)
        splitter.addWidget(self.log)

        splitter.setSizes([600, 180])             # Initial ratio: 600px top, 180px bottom
        layout.addWidget(splitter, 1)             # Add splitter to main layout, occupying remaining space

        # Automatically start loading the login page after setup
    ...
    def load_login_page(self):
        ...
        if not WEB_OK:
            self.log_message.emit("错误：无法加载页面，PyQtWebEngine 不可用。")
            return
        ...
        login_url_str = urllib.parse.urljoin(self.base_url, login_path)
    ...
    def _on_page_load_finished(self, ok: bool):
        ...
        if ok:
            ...
            # Check if logged in and auto-start worker
            if self.base_url and self.base_url in current_url and "/login/" not in current_url:
                self.log_message.emit("提示：检测到已登录状态，尝试自动开始同步...")
                self.extract_cookies_and_start_worker() # Auto-start here
            else:
                self.log_message.emit("提示：请在上方浏览器窗口登录 Moodle。登录成功后将自动开始同步。")
        else:
            error_string = self.web.property("loadErrorString")
            self.log_message.emit(f"错误：页面加载失败。 {error_string or ''}")

    # --- Cookie Extraction and Worker Start ---
    def extract_cookies_and_start_worker(self):
        """通过 JavaScript 获取 Cookies 并启动下载线程"""
        if not WEB_OK or not self.web.isVisible():
            self.log_message.emit("错误：浏览器视图不可用或未加载，无法提取 Cookies。")
            return

        # Prevent re-entry if worker is already running
        ...
        # Execute JavaScript to get document.cookie
        self.web.page().runJavaScript("document.cookie", self._on_js_cookies_received)

    def _on_js_cookies_received(self, js_cookie_string: str):
        """Callback after runJavaScript('document.cookie') finishes."""
        if not js_cookie_string:
        ...
        try:
            ...
            for item in js_cookie_string.split(';'):
                ...
                if '=' in item:
                    ...
                    # Basic URL decoding might be needed for some cookie values
                    name = urllib.parse.unquote(name)
                    value = urllib.parse.unquote(value)
                    cookie_dict[name] = value
                    self.log_message.emit(f"  解析 Cookie: {name}=***")
            if not cookie_dict:
                 raise ValueError("Cookie 字符串解析后为空，可能格式不正确或无有效 Cookie。")
            # Populate requests session cookies
            requests.utils.add_dict_to_cookiejar(self.session.cookies, cookie_dict)
            self.log_message.emit(f"从字符串解析并添加了 {len(cookie_dict)} 个 Cookie。")
        ...
        # === Add sys.path manipulation for note_downloader imports ===
        downloader_src_path = str(Path(self.project_root) / "src")
        if downloader_src_path not in sys.path:
            sys.path.insert(0, downloader_src_path)
            self._added_sys_path = downloader_src_path
        else:
    ...
    def _on_worker_finished(self):
        ...
        if hasattr(self, '_added_sys_path') and self._added_sys_path and self._added_sys_path in sys.path:
            try:
                sys.path.remove(self._added_sys_path)
                self.log_message.emit(f"清理 sys.path: 已移除 {self._added_sys_path}")
...
class DownloadWorker(QThread):
    ...
    def __init__(self, session, config, project_root, manifest_path, downloader_src_path, parent=None):
        ...
        self.downloader_src_path = downloader_src_path

    def run(self):
        """Main download logic executed in this thread."""
        added_path = False
        # Ensure the path is added within the thread's context if not already there
        ...
        try:
            ...
            import json
            import traceback
            # Use absolute imports assuming 'note_downloader' is discoverable
            # This relies on the main project structure or PYTHONPATH setup
            from note_downloader.src import utils as nd_utils
            from note_downloader.src import crawler as nd_crawler
            from note_downloader.src import downloader as nd_downloader
            from note_downloader.src.classifier import FileClassifier as NDFileClassifier
            from note_downloader.src import checker as nd_checker
            ...
            self.log_message.emit("--- Note Downloader (Thread) 开始运行 ---")
            self.log_message.emit("=" * 50)
            ...
            # Classifier needs path relative to its own location if using relative paths internally
            # Assuming project_root points to the 'note_downloader' base directory
            ...
            try:
                # Crawler needs base_url from cfg, not session
                if 'base_url' not in cfg:
                    self.log_message.emit("错误: 配置中缺少 'base_url'，无法获取课程列表。")
                    raise ValueError("配置中缺少 'base_url'")

                courses = nd_crawler.get_course_list(session, cfg)
                total_courses_found = len(courses)
                if not courses:
                    self.log_message.emit("警告: 未能从 Moodle 获取到任何课程列表。")
                else:
                 ...
                 return # Stop thread if course list fails

            # --- Prepare Download Directory ---
            download_dir_name = cfg.get("download_directory", "downloads")
            # base_download_dir should be relative to the note_downloader project root
            base_download_dir = Path(self.project_root) / download_dir_name
            self.log_message.emit(f"[步骤 2/3] 检查/创建基础下载目录: {base_download_dir}")
            try:
                 # Ensure safe_mkdir uses absolute path if needed, or handle CWD correctly
                ...
                for i, (course_name, course_url) in enumerate(courses):
                    ...
                    try:
                        ...
                        if not resource_links:
                            ...
                            # Pass the log_message signal's emit method as a logger callback
                            downloaded, skipped, failed = nd_downloader.bulk_download(
                                session,
                                resource_links,
                                str(course_download_folder),
                                manifest,
                                classifier_instance # Removed logger_func argument
                            )
            ...
            self.log_message.emit("=" * 50)
            self.log_message.emit("--- Note Downloader (Thread) 运行结束 ---")
